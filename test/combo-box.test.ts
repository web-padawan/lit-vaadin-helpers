...
import { comboBoxRenderer, ComboBoxModel } from '../src/combo-box-renderer.js';
  ...
  render() {
    return html`
      <vaadin-combo-box
        label="${this.label}"
        .items="${this.users}"
        .renderer="${comboBoxRenderer((model: ComboBoxModel<User>) => {
          return html`<b @click="${this.onItemClick}"
            >${model.item.name.first}${this.separator}${model.item.name.last}</b
          >`;
        }, this.separator)}"
